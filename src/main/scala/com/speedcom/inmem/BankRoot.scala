...
class BankRoot {
  ...
  var bankAccounts = Seq(
    ...
    BankAccount(ProfileId(2L), BankAccountNumber("3454"*5), "millionaire")
  )

  var transactionHistories = Seq(
    TransactionHistory(BankAccountNumber("5555"*5), transactions = List.empty),
    TransactionHistory(BankAccountNumber("3454"*5), transactions = List.empty),
    TransactionHistory(BankAccountNumber("1111"*5), transactions = List.empty)
  )
